...
{
  template <typename T>
  fooOp& operator&(const T& i) { std::cout << std::forward<T>(t); return *this; }
  void dump() {
    *this &1 & 2&3 & "4" & 5.0f &i_ &d_ &f_   &   s_;
  }
  int i_;
  double d_;
  float f_;
  ...
  auto process_return(ReturnT &&ret, output_terminalsT &out)
  {
    ...
    { // protect from compiling for
      ...
      if constexpr (std::is_same_v<returnT, ttg::resumable_task>)
      {
        ...
        { // if task is suspended return the coroutine promise ptr
          coro_handle = ret;
        }
        return coro_handle;
      }
        ...
        if (accumulate_ints_in_scratch)
        {
          ...
          if (deriv_order_ > 0)
          {
            ...
            default:
            {
              ...
              std::size_t s = 0;
              while (shellset_gaussian_diter)
              { // loop over derivs computed
                // by libint
...
#define BOOST_PP_NBODYENGINE_MCR3(r, product)                                                                        \
  ...
  {                                                                                                                  \
    ...
    BOOST_PP_IF(BOOST_PP_IS_1(LIBINT2_FLOP_COUNT),                                                                   \
                LIBINT2_PREFIXED_NAME(libint2_init_flopcounter)(&primdata_[0], primdata_.size()), BOOST_PP_EMPTY()); \
...
//  if ((!use_hrr && l2 > 0) || !hrr_ket_to_bra) {
    ...
    if (oper_is_nuclear)
    {
      ...
      const auto &params = (oper_ == Operator::nuclear) ? any_cast<const operator_traits<Operator::nuclear>::oper_params_type &>(params_) : std::get<1>(any_cast<const operator_traits<Operator::erfc_nuclear>::oper_params_type &>(params_));
...
#define BOOST_PP_NBODYENGINE_MCR8(r, data, i, elem)                              \
  case i:                                                                        \
    ...
    break;
      ...
      { // did not use scratch? may still need to update targets_
        if (set_targets_)
        {
          for (auto s = 0; s != ntargets; ++s)
